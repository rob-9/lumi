        ...
        for col, (title, caption) in zip(fig_cols, figure_placeholders):
            with col:
                with st.container(border=True, height=200):
                    st.markdown(f"**{title}**")
                    st.caption(caption)
                    st.caption(":gray[No figures generated yet]")
    ...
    with export_cols[0]:
        if st.button("Export PDF"):
            with st.spinner("Generating PDF..."):
                _pdf = asyncio.run(_generator.generate_pdf(report))
                st.download_button(
                    "Download PDF",
                    data=_pdf,
                    file_name=f"lumi_report_{report.query_id}.pdf",
                    mime="application/pdf",
                )
    with export_cols[1]:
        if st.button("Export HTML"):
            with st.spinner("Generating HTML..."):
                _html = asyncio.run(_generator.generate_html(report))
                st.download_button(
                    "Download HTML",
                    data=_html,
                    file_name=f"lumi_report_{report.query_id}.html",
                    mime="text/html",
                )
    with export_cols[2]:
        _json_str = _generator.generate_json(report)
        st.download_button(
            "Download JSON",
            data=_json_str,
            file_name=f"lumi_report_{report.query_id}.json",
